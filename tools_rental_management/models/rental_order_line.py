...
class RentalOrderLine(models.Model):
    ...
    unit_price = fields.Monetary(string='Price/Day')
    ...
    # Per-line period and duration (in days)
    ...
    planned_duration = fields.Float(
        string='Duration (Days)', default=1,
        help='Duration in days — auto-calculated from Per selection')
    ...
    # ── Auto-set price from pricing rule or product on create ──
    ...
    # ── Onchange: Per changed → auto-calculate duration in days ──
    @api.onchange('period_type')
    def _onchange_period_type(self):
        if self.period_type:
            self.planned_duration = DAY_MULTIPLIERS.get(self.period_type, 1)
    ...
    def _compute_costs(self):
        for line in self:
            ...
            # Base rental cost = price/day × planned days (qty is always 1)
            line.rental_cost = price * planned
