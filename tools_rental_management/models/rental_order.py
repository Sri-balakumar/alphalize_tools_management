...
class RentalOrder(models.Model):
    ...
    rental_duration = fields.Float(
        ...
        help='Duration in days — auto-calculated from Billing Period')
    ...
    def _compute_duration_display(self):
        for order in self:
            actual = order.actual_duration or 0
            planned = order.rental_duration or 0
            if actual and planned and actual > planned:
                extra = int(actual - planned)
                planned_int = int(planned)
                p_label = f"{planned_int} {'Day' if planned_int == 1 else 'Days'}"
                e_label = f"{extra} {'Day' if extra == 1 else 'Days'}"
                order.actual_duration_display = f"{p_label} + {e_label} Extra"
            elif actual:
                d = int(actual)
                order.actual_duration_display = f"{d} {'Day' if d == 1 else 'Days'}"
            elif planned:
                d = int(planned)
                order.actual_duration_display = f"{d} {'Day' if d == 1 else 'Days'}"
            else:
    ...
    def _onchange_rental_period_type(self):
        if self.rental_period_type:
            self.rental_duration = DAY_MULTIPLIERS.get(self.rental_period_type, 1)
    ...
    def action_checkin_wizard(self):
        ...
        for ol in self.line_ids:
            ...
            line_vals.append({
                ...
                'planned_days': (
                    ol.planned_duration
                    if ol.planned_duration
                    else (self.rental_duration or 1)
                ),
